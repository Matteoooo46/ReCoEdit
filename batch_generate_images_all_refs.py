...
# =====================================================================
# ★★★ 新增：持久化记录 API 调用的账本文件路径 ★★★
# 只要你不删这个文件，它就会一直记录每一天的用量
# =====================================================================
NOVITA_USAGE_LOG_FILE = "/data/phd/kousiqi/zhitao/novita_api_usage_log.json"

def record_novita_usage(api_key, is_success):
    """翻开账本，给今天的这个 Key 记上一笔"""
    today_str = datetime.now().strftime("%Y-%m-%d")
    # 为了安全和直观，只记录 Key 的前后几位 (例如: sk_uwo...jdVA)
    masked_key = f"{api_key[:6]}...{api_key[-4:]}"

    # 1. 尝试读取以前的旧账本
    usage_data = {}
    if os.path.exists(NOVITA_USAGE_LOG_FILE):
        try:
            with open(NOVITA_USAGE_LOG_FILE, 'r', encoding='utf-8') as f:
                usage_data = json.load(f)
        except Exception:
            pass # 如果文件损坏就重新开始

    # 2. 如果今天还没记过账，初始化今天的页面
    if today_str not in usage_data:
        usage_data[today_str] = {}
    if masked_key not in usage_data[today_str]:
        usage_data[today_str][masked_key] = {"total": 0, "success": 0, "fail": 0}

    # 3. 开始计费累加
    usage_data[today_str][masked_key]["total"] += 1
    if is_success:
        usage_data[today_str][masked_key]["success"] += 1
    else:
        usage_data[today_str][masked_key]["fail"] += 1

    # 4. 马上合上账本存进硬盘（防止断电或被 Kill 导致数据丢失）
    with open(NOVITA_USAGE_LOG_FILE, 'w', encoding='utf-8') as f:
        json.dump(usage_data, f, indent=4, ensure_ascii=False)


# =====================================================================
# ★★★ 基于 Novita API 的自动生图函数 (带自动记账) ★★★
# =====================================================================
def generate_novita_image(prompt, api_key, save_path, reference_image_paths=None): 
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json"
    }
    
    # 动态判断：如果传了参考图列表，就走 Edit 垫图接口
    if reference_image_paths and isinstance(reference_image_paths, list) and len(reference_image_paths) > 0:
        ...
        if img_base64_list:
            payload = {"prompt": prompt, "image_base64s": img_base64_list}
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "image_urls" in result and len(result["image_urls"]) > 0:
                ...
                # 💥 重点：请求成功，给这个 Key 记上一笔成功的账！
                record_novita_usage(api_key, is_success=True)
                return True
            else:
                logger.error("Novita 响应中未找到 image_urls")
                record_novita_usage(api_key, is_success=False) # 记失败的账
                return False
        else:
...
第一步：构思剧情大纲 (outline)
1. 人物设定 (characters):
- 设定符合目标受众的真实且具有高级感的人类形象（例如：自信从容的都市白领，穿着质感上乘的服饰）。 
...
第三步：创作分镜剧本 (script)
核心创意要求: 6个分镜必须形成一个连贯的叙事，以视觉方式转化优质素材结构化信息中的核心逻辑，全方位展示商品卖点。
...
## 输出格式要求
你的最终输出必须是一个完整的 JSON 对象，包含三个顶级键："outline", "characters_dressing", 和 "script"。请严格、逐字地遵循以下 JSON 结构骨架（仅替换尖括号 `<...>` 内的内容）：
## 输出格式示例
...
```json
{
  ...
  "characters_dressing": [
    {
      "id": "<string: 造型ID>",
      "character_id": "<string: 对应的人物ID>",
      "description": "<string: 详细的着装描述>"
    }
  ],
  "script": [
    {
      "shot_id": <int: 从 1 到 6>,
      "character_id": "<string: 引用的造型ID，无人物则必须严格输出 null>",
      "scene_id": "<string: 引用的场景ID>",
      "product_image_number": <array: 引用的商品图片编号数组，例如 [1]，无则必须严格输出 null>,
      "description": "<string: 纯视觉的、极具商业质感的画面详细描述>"
    }
  ]
}
```
...
if __name__ == '__main__':
    ...
    print("🔍 正在扫描硬盘，寻找已经造好的产品，请稍候...")
    if os.path.exists(root_dir):
        for folder_name in os.listdir(root_dir):
            workspace = os.path.join(root_dir, folder_name)
            # 我们把“最终剧本是否生成”作为这个商品彻底完工的标志
            script_path = os.path.join(workspace, "final_storyboard_script.json")
            item_info_path = os.path.join(workspace, "item_info.json")
            
            if os.path.exists(script_path) and os.path.exists(item_info_path):
                try:
                    with open(item_info_path, "r", encoding="utf-8") as f:
                        info = json.load(f)
                        completed_items.add(int(info["itemId"]))
                except Exception:
                    pass
                    
    print(f"✅ 检查完毕！发现 {len(completed_items)} 个商品已经造好，它们将被安全跳过。")
    
    extractor = ImageExtractor(root_dir=root_dir)
    ...
    for i, data in enumerate(my_data_list):
        ...
        if current_item_id in completed_items:
            print(f"[{i+1}/{len(my_data_list)}] ⏭️ 商品 ID: {current_item_id} 已存在，直接跳过！")
            continue
                    ...
                    if character_data and "description" in character_data:
                        ...
                        if gen_success:
                            ...
                            if new_script_json:
                                ...
                                # 🌟 1. 修改保存文件夹的名称，例如加上 _all_refs 后缀，避免与之前的覆盖
                                storyboard_images_save_dir = os.path.join(workspace, "storyboard_images_all_refs")
                                os.makedirs(storyboard_images_save_dir, exist_ok=True)
                                
                                # 依然保留历史帧滑动窗口（如果你想测试连贯性的话）
                                history_frames = [] 
                                ...
                                for idx, scene in enumerate(scenes_list):
                                    ...
                                    # ★★★ 核心中枢：无条件全量垫图策略 ★★★
                                    # =========================================================
                                    # 注意：这里我们不再去 get('character_id') 和 get('product_image_number') 了
                                    ...
                                    if history_frames:
                                        print(f"      🔍 剧本指令：已垫入【{len(history_frames)} 张历史帧】")
                                        
                                    # 🌟 2. 策略 2：无条件强制加入【人物定妆照】
                                    if final_image_save_path and os.path.exists(final_image_save_path):
                                        ref_imgs_for_novita.append(final_image_save_path)
                                        print(f"      🔍 剧本指令：强制垫入【人物定妆照】")
                                        
                                    # 🌟 3. 策略 3：无条件强制加入【商品原图】
                                    if result_img_paths and len(result_img_paths) > 0 and os.path.exists(result_img_paths[0]):
                                        ref_imgs_for_novita.append(result_img_paths[0])
                                        print(f"      🔍 剧本指令：强制垫入【商品原图】")
                                    ...
                                    scene_gen_success = generate_novita_image(
                                        ...
                                        reference_image_paths=ref_imgs_for_novita
                                    )
    ...
    # =========================================================
    print("\n" + "=" * 50)
    print("📊 每日 Nano Banana (Novita) API 调用账单")
    print("=" * 50)
    try:
        today_str = datetime.now().strftime("%Y-%m-%d")
        if os.path.exists(NOVITA_USAGE_LOG_FILE):
            ...
            if today_str in usage_data:
                ...
                for masked_key, stats in usage_data[today_str].items():
                    print(f"🔑 Key 尾号: {masked_key}")
                    print(f"   👉 累计总调用 : {stats['total']} 次")
                    print(f"   👉 其中成功   : {stats['success']} 次")
                    print(f"   👉 其中失败   : {stats['fail']} 次")
            else:
                print("今日暂无调用记录。")
    except Exception as e:
        print("账单读取失败。")
