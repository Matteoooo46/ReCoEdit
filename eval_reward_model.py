...
用法：
  python eval_reward_model.py              # 全部产品
  python eval_reward_model.py --dry-run 2  # 只跑前2个产品快速验证
"""
...
def get_product_name(prod_id):
    ...
    return prod_id  # workspace 类型直接用 ID
...
def main():
    ...
    # 收集所有有效目录（_data_enhanced_prompt_enhanced，不含 raw_）
    all_dirs = sorted([
    ...
    for idx, dirname in enumerate(valid_dirs, 1):
        ...
        print(f"  参考图: {ref_imgs[0]}  |  my={len(my_imgs)}张  nano={len(nano_imgs)}张")
        print(f"{'='*65}")
        ...
        for fname in my_imgs:
            ...
            score   = score_pair(client, prod_name, ref_image, gen_img)
            norm    = f"{(score-1)/4:.2f}" if score > 0 else " N/A"
            my_scores.append(score)
            print(f"    {fname:<20}  Score: {score}/5  归一化: {norm}")
    ...
    print(f"\n【p1_my（自己模型）】")
    print(f"  总张数: {len(valid_my)}")
    if valid_my:
        avg = sum(valid_my)/len(valid_my)
        dist = {i: valid_my.count(i) for i in range(1,6)}
        print(f"  全局均分: {avg:.3f}/5  (归一化: {(avg-1)/4:.3f})")
        print(f"  分布: 1分={dist[1]}  2分={dist[2]}  3分={dist[3]}  4分={dist[4]}  5分={dist[5]}")

    print(f"\n【p1_nano（nano banana）】")
    print(f"  总张数: {len(valid_nano)}")
    if valid_nano:
