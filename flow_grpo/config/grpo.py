...
def counting_qwenimage_edit_8gpu():
    ...
    # flux
    config.pretrained.model = "/data/phd/hf_models/ckpt/Qwen/Qwen-Image-Edit"
    config.sample.num_steps = 10
    config.sample.eval_num_steps = 50
    config.sample.guidance_scale = 4
...
def counting_qwenimage_edit_8gpu_product_consistency():
    gpu_number = 8
    config = compressibility()
    config.dataset = "/data/phd/jinjiachun/zzt/dual_grpo/flow_grpo/dataset/product_consistency"
    ...
    config.save_freq = 4
    config.eval_freq = 30
    config.save_dir = 'logs/product_consistency/qwenimage_edit_round4'
    config.train.transformer_path = "/data/phd/kousiqi/zhitao/full_all_products_rewrite_expanded_ultimate_with_ultraedit500k_captioned_resume_112000/step-10000.safetensors"
    config.reward_fn = {
        "product_consistency": 1.0,
    }
    config.per_prompt_stat_tracking = True
    return config
