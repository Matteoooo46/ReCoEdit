...
class GenevalPromptImageDataset(Dataset):
    def __init__(self, dataset, split='train'):
        ...
        # Support datasets that use 'val' instead of 'test' for evaluation split
        candidate = os.path.join(dataset, f'{split}_metadata.jsonl')
        if not os.path.exists(candidate) and split == 'test':
            candidate = os.path.join(dataset, 'val_metadata.jsonl')
    ...
    def _load_image(self, meta):
        # Support both old format ('image': relative path) and new format ('images': list of absolute paths)
        if 'images' in meta:
            image_path = meta['images'][0]
            image_key = image_path  # already absolute
    ...
    def __getitem__(self, idx):
        ...
        image, image_key = self._load_image(meta)
        item = {
            "prompt": self.prompts[idx],
            "metadata": meta,
            "prompt_with_image_path": f"{self.prompts[idx]}_{image_key}",
            "image": image,
        }
        return item
...
def main(_):
    ...
    # Load SFT transformer checkpoint if provided (before LoRA wrapping, so LoRA learns on top of SFT)
    if config.train.transformer_path:
        if rank == 0:
            logger.info(f"Loading SFT transformer weights from {config.train.transformer_path}")
        sft_state_dict = load_file(config.train.transformer_path)
        pipeline.transformer.load_state_dict(sft_state_dict, strict=True)
        del sft_state_dict
    if config.use_lora:
    ...
    while True:
        ...
        if rank == 0:
            wandb.log(
                {
                    "epoch": epoch,
                    **{f"reward_{key}": value.mean() for key, value in gathered_rewards.items() if '_strict_accuracy' not in key and '_accuracy' not in key and key != 'product_mask'},
                },
                step=global_step,
            )

        # --- Log skip ratio when product_mask is present ---
        if "product_mask" in gathered_rewards:
            valid_mask = gathered_rewards["product_mask"] > 0.5
            n_total = len(valid_mask)
            n_valid = valid_mask.sum()
            if rank == 0:
                print(f"[product_mask] {n_valid}/{n_total} valid, {n_total - n_valid} skipped")
                wandb.log({"skip_ratio": (n_total - n_valid) / max(n_total, 1)}, step=global_step)
