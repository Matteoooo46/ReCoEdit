...
def generate_image_caption(image_path, retry_count=3):
    ...
    # 每个进程独立创建 client，避免多进程共享 httpx 连接
...
# 改进：主进程预加载 JSON 并切分，子进程直接接收数据切片，避免重复加载
# ==========================================
def run_optimized_on_single_process(global_rank, my_assigned_data, temp_output_dir):
    assigned_count = len(my_assigned_data)
    print(f"[Rank {global_rank}] 分到 {assigned_count} 条数据，开始处理...")
    ...
    if os.path.exists(my_temp_output_json):
        try:
            ...
            print(f"[Rank {global_rank}] 断点续传：已恢复 {len(my_clean_workspace_data)} 条。")
        except Exception as e:
            print(f"[Rank {global_rank}] 历史文件损坏，从头开始: {e}")
            my_clean_workspace_data = []
    ...
    for idx, item in enumerate(my_assigned_data):
        ...
        if idx % 50 == 0:
            print(f"   [Rank {global_rank}] 进度: {idx}/{assigned_count}")
        ...
        if len(my_clean_workspace_data) > 0 and len(my_clean_workspace_data) % 2000 == 0:
            ...
            print(f"[Rank {global_rank}] 已达 2000 条，阶段性存盘！")

    with open(my_temp_output_json, 'w', encoding='utf-8') as f:
        json.dump(my_clean_workspace_data, f, ensure_ascii=False, indent=2)

    print(f"[Rank {global_rank}] 完成！处理 {processed_count} 条，跳过 {skipped_count} 条。")

# ==========================================
# 结果聚合
# ==========================================
def reconstruct_final_json(total_world_size, temp_output_dir, output_final_json):
    print(f"开始合并 {total_world_size} 个分片...")
    ...
    print(f"合并完成！共 {len(final_optimized_data)} 条数据。")
    print(f"保存在: {output_final_json}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="分布式打标集群版 v3（内存优化 + 高并发支持）")
    parser.add_argument("--node_rank", type=int, required=True, help="当前机器编号 (0 到 N-1)")
    parser.add_argument("--procs_per_node", type=int, default=64, help="每台机器的进程数 (默认 64)")
    parser.add_argument("--total_nodes", type=int, default=5, help="集群总机器数 (默认 5)")
    ...
    total_world_size = args.total_nodes * args.procs_per_node

    if args.merge_only:
        reconstruct_final_json(total_world_size, TEMP_OUTPUT_DIR, OUTPUT_FINAL_JSON)
    else:
        print(f"集群启动！Node {args.node_rank}/{args.total_nodes - 1}, 全局总进程: {total_world_size}")

        # 主进程只加载一次 JSON，预切分后传给子进程
        print("主进程加载 JSON 数据...")
        with open(ORIGINAL_JSON_PATH, 'r', encoding='utf-8') as f:
            all_data = json.load(f)
        print(f"共 {len(all_data)} 条数据，开始切分...")

        # 预切分：每个子进程拿自己的数据切片
        slices = {}
        for local_rank in range(args.procs_per_node):
            global_rank = (args.node_rank * args.procs_per_node) + local_rank
            slices[global_rank] = all_data[global_rank::total_world_size]
        ...
        for local_rank in range(args.procs_per_node):
            ...
            p = mp.Process(
                ...
                args=(global_rank, slices[global_rank], TEMP_OUTPUT_DIR)
            )
            p.start()
            processes.append(p)
        ...
        print(f"Node {args.node_rank} 的 {args.procs_per_node} 个进程全部完成！")
