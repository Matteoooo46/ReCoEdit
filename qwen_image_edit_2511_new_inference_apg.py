...
def parse_args():
    parser = argparse.ArgumentParser(description="Qwen-Image-Edit 单产品推理脚本 (APG版)")
...
# ==========================================
# ⚠️ 填入你要推理的那个产品的文件夹路径
TARGET_WORKSPACE_DIR = cmd_args.workspace_dir or "/data/phd/kousiqi/zhitao/new_validation_set/08936a21-6a4a-40c1-828f-f30763afdf02"
...
# 是否启用 QwenVL prompt 改写；如要回到原始逻辑，改成 False 即可
ENABLE_PROMPT_REWRITE = True

# ==========================================
# 1.1 QwenVL Prompt 改写模型配置（来自 test_qwenvl.py 的用法）
# ==========================================
...
def rewrite_prompt_with_qwenvl(original_prompt, ref_image_paths):
    ...
    try:
        ...
        return rewritten_prompt if rewritten_prompt else original_prompt
    except Exception as e:
...
# lora_path = "/data/phd/kousiqi/zhitao/full_all_products_rewrite_expanded_ultimate_with_ultraedit500k_resume_88000/step-22000.safetensors"
...
patch_pipeline_for_apg(
    ...
    norm_threshold=None,  # Norm clipping, None = 禁用
)
...
if not found_reference_dir or not found_target_dir:
    raise ValueError(f"❌ 未能在 {TARGET_WORKSPACE_DIR} 中找齐 reference_imgs 和 augmented_generated_images 文件夹！")
...
edit_image_list = []
viewer_globals = []
report_folder_name = os.path.basename(REPORT_DIR)
...
for idx, ref_img in enumerate(ref_imgs):
    ...
    viewer_globals.append({"name": f"产品参考{idx + 1}", "path": f"{report_folder_name}/{report_ref_name}"})

print(f"✅ 参考图片已整理至 Report 文件夹。用于推理的图片共 {len(edit_image_list)} 张。")

# QwenVL 改写时使用的商品参考图列表（推理时不依赖目标图）
...
# ==========================================
frame_local_paths = []  # list of (nano_path, my_path)
...
ref_file_paths = [os.path.join(found_reference_dir, r) for r in ref_imgs]
...
# 用公网 URL 重建 frames
for log_idx, log in enumerate(prompt_rewrite_logs):
