...
def test_prompt_classification(client):
    ...
    for i, prompt in enumerate(all_prompts):
        ...
        expected_str = "Yes" if expected[i] else "No"
        try:
            ...
            ok = "PASS" if predicted == expected[i] else "FAIL"
            if predicted == expected[i]:
                correct += 1
            print(f"  [{ok}] 期望={expected_str} 预测={raw:<4}  |  {prompt[:70]}...")
        except Exception as e:
            print(f"  [ERR] {prompt[:70]}... → {e}")

    print(f"\n  准确率: {correct}/{len(all_prompts)} ({100*correct/len(all_prompts):.0f}%)")
...
def test_image_scoring(client, dry_run=None):
    ...
    for idx, dirname in enumerate(valid_dirs, 1):
        ...
        if scores:
            ...
            print(f"    Yes={n_yes}  No={n_no}  Yes率={n_yes/len(valid):.1%}")
...
def main():
    ...
    try:
        ...
        print(f"OK (模型: {models.data[0].id})")
    except Exception as e:
        print(f"失败: {e}")
